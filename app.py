...
def get_last_password_number(clinic_id):
    """Retorna o número da última senha gerada para uma clínica específica."""
    today = datetime.now(timezone.utc).date()
    last_password = Password.query.filter_by(date=today, clinic_id=clinic_id).order_by(Password.number.desc()).first()
...
def generate_password(data):
    """Gera uma nova senha e emite para a sala da clínica correspondente."""
    ...
    last_password = get_last_password_number(clinic_id)
    new_password = last_password + 1
    ...
    print(f"Emitindo nova senha {new_password} para a clínica {clinic_id}")
